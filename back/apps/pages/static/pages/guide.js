...
async function refreshExerciseReactions() {
    const data = await getExerciseReactions();
...
function startReactionPolling() {
    if (reactionPollTimer) {
        clearInterval(reactionPollTimer);
    }

    reactionPollTimer = setInterval(() => {
        refreshExerciseReactions().catch((error) => {
            console.warn(error.message || "Не удалось обновить реакции");
        });
    }, 5000);
}
...
async function initGuidePage() {
    ...
    renderMuscleGroups();
    await refreshExerciseReactions();
